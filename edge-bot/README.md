...
High-turnover/possible market-maker wallets are kept in a separate research cohort. Their positions do **not** vote in the score: the bot records 5/15-minute changes in their aggregate signed inventory, then `evaluate-scalps` tests both continuation and fade interpretations. That avoids pretending a hedged maker's current inventory is automatically a price forecast.
